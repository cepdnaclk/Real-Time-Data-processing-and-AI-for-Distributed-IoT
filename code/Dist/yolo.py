...
def conv_forward(A_prev, W, b, hparameters):
    ...
    with pymp.Parallel(2) as p: 
      for i in p.range(m):               # loop over the batch of training examples
        a_prev_pad = A_prev_pad[i]               # Select ith training example's padded activation
        for h in p.range(n_H):           # loop over vertical axis of the output volume
            vert_start = h*stride
            vert_end = vert_start+f

            for w in p.range(n_W):       # loop over horizontal axis of the output volume
        ...
        #iterate over the filters
            #with pymp.Parallel(2) as p: 
                for c in p.range(n_C):   # loop over channels (= #filters) of the output volume
